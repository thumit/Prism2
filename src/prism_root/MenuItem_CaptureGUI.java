		...
		addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				if (getText().equals("5s Image Capture")) {			// This is to not create new thread when the old thread is still going
					...
					int response = JOptionPane.showOptionDialog(Prism2Main.get_Prism_DesktopPane(), warningText, "Image Capture",
						...
						Thread thread = new Thread() { // Make a thread to allow capture screen within 5 seconds
							public void run() {
								if (!isSelected()) {
									...
									for (Component comp : getAllComponents(Prism2Main.get_main())) {
